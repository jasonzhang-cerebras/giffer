...
export const GifferCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs
      ...
      .option("source", {
        alias: ["s"],
        describe:
          "image source to use (lorem-picsum, placeholder, emoji, canvas)",
        type: "string",
      })
      .option("all", {
  ...
  handler: async (args) => {
    try {
      UI.println(
        UI.Style.TEXT_INFO_BOLD + "🎨 Giffer - Multi-Source Picture Generator",
      );
      UI.println();
      ...
      UI.println(
        UI.Style.TEXT_SUCCESS + "✓ Keywords extracted:",
        keywords.join(", "),
      );
      UI.println();
      ...
      if (useAllSources) {
        UI.println(
          UI.Style.TEXT_DIM + "Generating pictures from all sources...",
        );
        UI.println();
        ...
        for (const [sourceName, imageUrls] of allImages.entries()) {
          ...
          UI.println(
            UI.Style.TEXT_SUCCESS + `✓ Generated ${sourceName}: ${outputPath}`,
          );
        ...
        UI.println(
          UI.Style.TEXT_DIM + `Generating picture from ${sourceName}...`,
        );
        UI.println();

        const imageUrls = await giffer.generateFromSource(sourceName, keywords);
        const imageUrl = imageUrls[0];
        const outputPath =
          args.output ?? `giffer-${sourceName}-${Date.now()}.png`;
        await giffer.generateImage(imageUrl, outputPath);
        ...
        UI.println(
          UI.Style.TEXT_DIM + "Generating picture from default source...",
        );
        UI.println();
